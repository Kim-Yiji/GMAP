...
class social_dmrgcn(nn.Module):
    def __init__(self, n_stgcn=1, n_tpcnn=4, input_feat=2, output_feat=5, seq_len=8, pred_seq_len=12, kernel_size=3):
        ...
        # Disentangling Scale Set per relation:
        # [R_PP^disp, R_PP^dist, R_PP^threat, R_PO^threat]
        split = [
            [0, 1/4, 2/4, 3/4, 1],      # displacement magnitude bins (normalized)
            [0, 1/2, 1, 2, 4],          # distance bins in meters
            [0.2, 0.4, 0.6, 0.8],       # threat score bins for PP threat
            [0.2, 0.4, 0.6, 0.8],       # threat score bins for PO threat
        ]

        # GCN Block
        self.st_dmrgcns = nn.ModuleList()
        self.st_dmrgcns.append(st_dmrgcn(input_feat, output_feat, (kernel_size, seq_len), split=split, relation=4))
        for j in range(1, self.n_stgcn):
            self.st_dmrgcns.append(st_dmrgcn(output_feat, output_feat, (kernel_size, seq_len), split=split, relation=4))
