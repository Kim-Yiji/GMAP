...
from model import *
from utils import TrajectoryDataset, SDDTrajectoryDataset, data_sampler
...
parser.add_argument('--dataset', default='eth', help='Dataset name or SDD scene (bookstore, coupa, ...)')
parser.add_argument('--dataset_type', default='generic', choices=['generic', 'sdd'])
parser.add_argument('--labels_dir', default=None, help='Directory with <video>_labels.csv (track_id,label)')
parser.add_argument('--class_size_csv', default=None, help='CSV with class sizes: label,size')
...
parser.add_argument('--compute_metrics', action="store_true", default=False, help='Compute ADE/FDE during validation')
parser.add_argument('--kstpes', type=int, default=20, help='Num samples for ADE/FDE')
...
if args.dataset_type == 'sdd':
    dataset_path = './sdd_datasets/' + args.dataset + '/'
checkpoint_dir = './checkpoints/' + args.tag + '/'

if args.dataset_type == 'sdd':
    train_dataset = SDDTrajectoryDataset(dataset_path + 'train/', obs_len=args.obs_seq_len, pred_len=args.pred_seq_len, skip=1,
                                         labels_dir=args.labels_dir, class_size_csv=args.class_size_csv)
else:
...
if args.dataset_type == 'sdd':
    val_dataset = SDDTrajectoryDataset(dataset_path + 'val/', obs_len=args.obs_seq_len, pred_len=args.pred_seq_len, skip=1,
                                       labels_dir=args.labels_dir, class_size_csv=args.class_size_csv)
else:
...
def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Handle track_ids if present (for SDD dataset)
        if len(batch) > 10:
            V_obs, A_obs, V_tr, A_tr = [tensor.cuda() for tensor in batch[-5:-1]]
        else:
...
def valid(epoch):
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        if args.compute_metrics:
            ...
            temp = (V_absl - V_pred_traj_gt).pow(2).sum(dim=-1).sqrt()
            ade_batch = temp.mean(dim=1).min(dim=0)[0]
            fde_batch = temp[:, -1, :].min(dim=0)[0]
            ade_all.extend(ade_batch.tolist())
            fde_all.extend(fde_batch.tolist())
    ...
    if args.compute_metrics and len(ade_all) > 0:
        ade_ = sum(ade_all) / len(ade_all)
        fde_ = sum(fde_all) / len(fde_all)
        writer.add_scalar('Metrics/ADE', ade_, epoch)
        writer.add_scalar('Metrics/FDE', fde_, epoch)
