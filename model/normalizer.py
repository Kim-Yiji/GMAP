...
def normalized_adjacency_matrix(A):
    ...
    norm_degs_matrix = torch.eye(A.size(-1), device=A.device) * degs_inv_sqrt
...
def normalized_adjacency_tilde_matrix(A):
    ...
    A_t = A + torch.eye(A.size(-1), device=A.device)
...
def normalized_laplacian_matrix(A):
    ...
    return torch.eye(A.size(-1), device=A.device) - normalized_adjacency_matrix(A)
...
def normalized_laplacian_tilde_matrix(A):
    ...
    A_t = A + torch.eye(A.size(-1), device=A.device)
    return torch.eye(A_t.size(-1), device=A.device) - normalized_adjacency_matrix(A_t)
